...
# FUNCIÓN MEJORADA CON MODELOS RECOMENDADOS POR LITERATURA USGS/WMO
def ajustar_modelo_relacion(x, y, nombre_relacion):
    """Ajustar modelos basados en recomendaciones de USGS/WMO/literatura"""
    
    # Definir modelos específicos por tipo de relación
    if nombre_relacion == "Altura-Caudal":
        modelos = [
            ('Potencial (USGS)', func_pot),           # Más común en ríos naturales
            ('Polinómico G2', func_poly2),           # Recomendado USGS
            ('Polinómico G3', func_poly3),           # Para comportamientos complejos
            ('Exponencial', func_exp),               # Alternativa
        ]
        
    elif nombre_relacion == "Altura-Velocidad":
        modelos = [
            ('Logarítmico (Teórico)', func_log),     # Perfil turbulento - RECOMENDADO
            ('Potencial', func_pot),                 # Relación de potencia
            ('Lineal', lambda x, a, b: a * x + b),  # Aproximación simple
            ('Polinómico G2', func_poly2),           # Para no linealidades
        ]
        
    elif nombre_relacion == "Altura-Área":
        modelos = [
            ('Polinómico G2 (USGS)', func_poly2),    # Geometría de canal - RECOMENDADO
            ('Lineal', lambda x, a, b: a * x + b),  # Canales rectangulares
            ('Potencial', func_pot),                 # Canales naturales
            ('Polinómico G3', func_poly3),           # Geometrías complejas
        ]
        
    elif nombre_relacion == "Caudal-Velocidad":
        modelos = [
            ('Lineal (Continuidad)', lambda x, a, b: a * x + b),  # Q = VA - RECOMENDADO
            ('Potencial', func_pot),                 # Comportamiento no lineal
            ('Polinómico G2', func_poly2),           # Relación cuadrática
        ]
    
    else:
        # Modelos por defecto para relaciones no especificadas
        modelos = [
    ...
    for nombre, funcion in modelos:
        try:
            ...
            elif nombre == 'Logarítmico (Teórico)':
                # Asegurar que x sea positivo para logaritmo + ajuste teórico
                x_positivo = np.maximum(x, 0.001)  # Evitar log(0)
                params, _ = curve_fit(funcion, x_positivo, y, p0=[1.0, 1.0], maxfev=5000)
            elif nombre == 'Potencial (USGS)':
                # Ajuste inicial común en hidrología
                params, _ = curve_fit(funcion, x, y, p0=[1.0, 1.5], maxfev=5000)
            ...
            if r2 > mejor_r2 and r2 > 0:  # Aceptar modelos con R² > 0
                mejor_r2 = r2
                mejor_modelo = nombre
                mejor_params = params
...
# FUNCIÓN PARA AGREGAR FUNDAMENTOS TEÓRICOS A LOS GRÁFICOS
def agregar_fundamento_teorico(ax, relacion):
    """Agregar fundamento teórico basado en literatura"""
    fundamentos = {
        "Altura-Caudal": "USGS: Q ∝ Hⁿ (n≈1.5-2.5)\nManning: Q ∝ H⁵/³",
        "Altura-Velocidad": "Flujo turbulento: V ∝ ln(H)\nPerfil logarítmico",
        "Altura-Área": "Geometría del canal\nA ∝ H² (parabólico)",
        "Caudal-Velocidad": "Ecuación continuidad: Q = VA\nRelación lineal esperada"
    }
    
    if relacion in fundamentos:
        ax.text(0.02, 0.98, fundamentos[relacion], transform=ax.transAxes,
...
def ajustar_curva(datos_grupo):
    ...
    # MODELOS RECOMENDADOS POR USGS/WMO PARA CURVAS H-Q
    modelos = [
        ('Potencial (USGS)', func_pot),      # Más común en ríos naturales
        ('Polinómico G2', func_poly2),       # Recomendado USGS
        ('Polinómico G3', func_poly3),       # Para comportamientos complejos
    ...
    for nombre, funcion in modelos:
        try:
            if nombre == 'Potencial (USGS)':
                params, _ = curve_fit(funcion, H_sorted, Q_sorted, p0=[1.0, 1.5], maxfev=5000)
            else:
...
def crear_grafico_principal(df, curvas, titulo):
    ...
    ax.set_title(f'{titulo}\n(Modelos USGS/WMO)', fontsize=14, fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Agregar fundamento teórico
    ax.text(0.02, 0.98, "USGS: Q ∝ Hⁿ (n≈1.5-2.5)\nManning: Q ∝ H⁵/³", 
            transform=ax.transAxes, fontsize=9, verticalalignment='top',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    
    return fig

# FUNCIÓN MEJORADA PARA GRÁFICOS COMPLEMENTARIOS CON MODELOS USGS/WMO
def crear_graficos_complementarios(df, titulo_sufijo=""):
    """Crear gráficos complementarios con modelos recomendados por literatura USGS/WMO"""
    ...
    # 1. Altura vs Velocidad - CON FUNDAMENTO TEÓRICO
    ax1 = axes[0, 0]
    ...
    # Ajustar modelo RECOMENDADO por literatura
    ...
    if modelo_vel and r2_vel > 0:
        ...
        if 'Logarítmico' in modelo_vel:
            y_pred_vel = funcion_vel(np.maximum(x_range_vel, 0.001), *params_vel)
        else:
    ...
    # Agregar fundamento teórico
    agregar_fundamento_teorico(ax1, "Altura-Velocidad")
    
    # 2. Altura vs Área - CON FUNDAMENTO TEÓRICO
    ax2 = axes[0, 1]
    ...
    if modelo_area and r2_area > 0:
        ...
        if 'Logarítmico' in modelo_area:
            y_pred_area = funcion_area(np.maximum(x_range_area, 0.001), *params_area)
        else:
    ...
    # Agregar fundamento teórico
    agregar_fundamento_teorico(ax2, "Altura-Área")
    ...
    if modelo_rh and r2_rh > 0:
        ...
        if 'Logarítmico' in modelo_rh:
            y_pred_rh = funcion_rh(np.maximum(x_range_rh, 0.001), *params_rh)
        else:
    ...
    # 4. Caudal vs Velocidad - CON FUNDAMENTO TEÓRICO
    ax4 = axes[1, 1]
    ...
    if modelo_caudal and r2_caudal > 0:
        ...
        if 'Logarítmico' in modelo_caudal:
            y_pred_caudal = funcion_caudal(np.maximum(x_range_caudal, 0.001), *params_caudal)
        else:
    ...
    # Agregar fundamento teórico
    agregar_fundamento_teorico(ax4, "Caudal-Velocidad")
...
if opcion == "🏠 Inicio":
    ...
    st.info("Aplicacion IA para generar curvas altura-caudal usando IA - Basado en estándares USGS/WMO")
    
    st.subheader("📚 Fundamentos Teóricos Utilizados")
    st.markdown("""
    **Modelos basados en recomendaciones de:**
    - **USGS** (United States Geological Survey)
    - **WMO** (World Meteorological Organization)
    - **Manuales de hidrometría internacional**
    
    **Relaciones hidráulicas implementadas:**
    - **Altura-Caudal**: Modelos Potencial (Q ∝ Hⁿ) y Polinómicos
    - **Altura-Velocidad**: Modelo Logarítmico (perfil turbulento)
    - **Altura-Área**: Modelo Polinómico (geometría de canal)
    - **Caudal-Velocidad**: Modelo Lineal (ecuación de continuidad)
    """)
    ...
    **Columnas requeridas en CSV:**
    ...
    st.info("**Sistema basado en estándares USGS/WMO** - Los modelos utilizados siguen recomendaciones internacionales")
                    ...
                    if st.session_state.procesamiento_realizado and st.session_state.curvas_sin_alto_rh is not None:
                        ...
                        for grupo, curva in curvas_sin.items():
                            ...
                            with st.expander(f"{grupo} - R² = {curva['r2']:.3f} - {rango_formateado} m"):
                                ...
                                elif curva['nombre'] == 'Potencial (USGS)':
                        ...
                        st.info("**Modelos basados en recomendaciones USGS/WMO**")
    ...
    st.info("**Sistema basado en estándares USGS/WMO**")
        ...
        if st.button("🚀 Procesar Datos Manuales", type="primary") and datos_manual:
            with st.spinner("Procesando datos manuales..."):
                ...
                if curvas:
                    ...
                    # Gráficos complementarios para datos manuales
    ...
    st.info("Esta sección muestra información sobre las curvas del modelo basado en estándares USGS/WMO")
    ...
    if modelo is None:
        ...
        # Información sobre modelos USGS/WMO
        st.subheader("📚 Modelos Utilizados (USGS/WMO)")
        st.markdown("""
        **Curvas Altura-Caudal:**
        - **Potencial (USGS)**: Q = aHᵇ - Más común en ríos naturales
        - **Polinómico G2**: Q = aH² + bH + c - Recomendado USGS
        - **Polinómico G3**: Q = aH³ + bH² + cH + d - Comportamientos complejos
