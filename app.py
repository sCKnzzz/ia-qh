...
def crear_grafico_profesional(ax, x, y, xlabel, ylabel, titulo, color='blue', marca='o', tamano_marca=60):
...
def agregar_ecuacion_grafico(ax, x_pos, y_pos, ecuacion, fontsize=9):
...
def crear_graficos_complementarios(df_procesado, curvas):
    ...
    fig.suptitle('Relaciones Hidráulicas - Análisis Complementario', fontsize=14, fontweight='bold')
                    ...
                    if st.button("🚀 Procesar Aforos", type="primary"):
                        with st.spinner("Procesando aforos y generando curvas..."):
                            try:
                                ...
                                if curvas:
                                    # GRÁFICO PRINCIPAL MEJORADO - Curvas Altura-Caudal
                                    st.subheader("📈 Curvas Altura-Caudal Generadas")
                                    fig_principal, ax_principal = plt.subplots(figsize=(10, 6))
                                    ...
                                    for grupo, curva in curvas.items():
                                        ...
                                        ax_principal.scatter(grupo_data['NIVEL_AFORO'], grupo_data['CAUDAL'], 
                                                          color=color, marker=marcador, s=80, label=grupo, 
                                        ...
                                        ax_principal.plot(H_range, Q_curve, color=color, linewidth=2.5, 
                                        ...
                                        ax_principal.text(x_pos, y_pos, ecuacion, fontsize=9,
                                                       bbox=dict(boxstyle="round,pad=0.3", facecolor=color, alpha=0.2))
                                    
                                    ax_principal.set_xlabel('Nivel (m)', fontsize=11, fontweight='bold')
                                    ax_principal.set_ylabel('Caudal (m³/s)', fontsize=11, fontweight='bold')
                                    ax_principal.set_title('Curvas Altura-Caudal por Grupo', fontsize=12, fontweight='bold')
                                    ax_principal.legend(fontsize=9)
