...
def crear_graficos_complementarios(df_procesado, curvas):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(10, 8))  # REDUCIDO DE 12,10 A 10,8
...
if opcion == "🏠 Inicio":
    ...
    **🎯 Funcionalidades:**
                        ...
                        with st.spinner("Procesando aforos y generando curvas..."):
                            try:
                                ...
                                if curvas:
                                    ...
                                    fig_principal, ax_principal = plt.subplots(figsize=(8, 5))  # REDUCIDO DE 10,6 A 8,5
                                    ...
                                    for grupo, curva in curvas.items():
                                        ...
                                        ax_principal.scatter(grupo_data['NIVEL_AFORO'], grupo_data['CAUDAL'], 
                                                          color=color, marker=marcador, s=60, label=grupo,  # REDUCIDO s=80 A s=60
                                        ...
                                        ax_principal.text(x_pos, y_pos, ecuacion, fontsize=8,  # REDUCIDO fontsize=9 A 8
                                                       bbox=dict(boxstyle="round,pad=0.3", facecolor=color, alpha=0.2))
                                    
                                    ax_principal.set_xlabel('Nivel (m)', fontsize=10, fontweight='bold')  # REDUCIDO fontsize
                                    ...
                                    ax_principal.legend(fontsize=8)  # REDUCIDO fontsize
                                    ...
                                    # GRÁFICOS COMPLEMENTARIOS (TAMAÑO YA REDUCIDO EN LA FUNCIÓN)
    ...
    if modelo is None:
        ...
        # Ingreso manual (código similar al anterior pero con ajustes de tamaño)
        ...
        if st.button("🚀 Procesar") and nuevos_datos:
            try:
                ...
                if curvas:
                    ...
                    fig, ax = plt.subplots(figsize=(8, 5))  # REDUCIDO
                    colores = {'GRUPO_ALTO_RH': 'red', 'GRUPO_RECIENTE': 'blue', 'GRUPO_ESTANDAR': 'green'}
                    
                    for grupo, curva in curvas.items():
                        color = colores.get(grupo, 'orange')
                        grupo_data = clasificados[clasificados['GRUPO_PREDICHO'] == grupo]
                        ax.scatter(grupo_data['NIVEL_AFORO'], grupo_data['CAUDAL'], color=color, s=60, label=grupo)  # REDUCIDO
                        
                        H_range = np.linspace(curva['rango_niveles'][0]*0.9, curva['rango_niveles'][1]*1.1, 100)
                        Q_curve = curva['funcion'](H_range, *curva['parametros'])
                        ax.plot(H_range, Q_curve, color=color, linewidth=2, label=f"{grupo} (R²={curva['r2']:.3f})")
                    
                    ax.set_xlabel('Nivel (m)', fontsize=10)  # REDUCIDO
                    ax.set_ylabel('Caudal (m³/s)', fontsize=10)
                    ax.legend(fontsize=8)  # REDUCIDO
