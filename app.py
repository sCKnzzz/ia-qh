...
def predecir_curvas_con_filtro(modelo, nuevos_datos, incluir_alto_rh=True):
    ...
    if not incluir_alto_rh:
        ...
        if 'GRUPO_ALTO_RH' in df_filtrado['GRUPO_PREDICHO'].values:
            st.info(f"✅ Se incluyó GRUPO_ALTO_RH. Aforos totales: {len(df_filtrado)}")
                    ...
                    if st.button("🚀 Procesar Aforos", type="primary"):
                        with st.spinner("Procesando aforos y generando curvas..."):
                            try:
                                ...
                                curvas, clasificados = predecir_curvas_con_filtro(modelo, df_subido, incluir_alto_rh=False)
