...
# FUNCIÓN COMPLETA PARA PREPARAR DATOS (INDEPENDIENTE DEL MODELO)
def preparar_datos_para_modelo(df):
    """Preparar datos para el modelo - Función independiente"""
...
# FUNCIÓN COMPLETA PARA PREDECIR CURVAS CON FILTRO
def predecir_curvas_con_filtro_completo(modelo, nuevos_datos, incluir_alto_rh=True):
    """Función completa para predecir curvas con opción de filtrado"""
    if modelo.clasificador is None:
        raise ValueError("El modelo debe ser entrenado primero")
    
    # Preparar datos usando función independiente
    df_procesado = preparar_datos_para_modelo(nuevos_datos)
    
    # Definir features (deben coincidir con las usadas en el entrenamiento)
    features = [
    ...
    for feature in features:
        if feature not in df_procesado.columns:
            st.error(f"❌ Falta la feature: {feature}")
            return {}, df_procesado
    ...
    if not incluir_alto_rh:
        ...
        if 'GRUPO_ALTO_RH' in df_procesado['GRUPO_PREDICHO'].values:
            st.info(f"📊 Se excluyó GRUPO_ALTO_RH. Aforos utilizados: {len(df_filtrado)} de {len(df_procesado)}")
    else:
        df_filtrado = df_procesado.copy()
        if 'GRUPO_ALTO_RH' in df_filtrado['GRUPO_PREDICHO'].values:
            st.info(f"📊 Se incluyó GRUPO_ALTO_RH. Aforos utilizados: {len(df_filtrado)}")
...
def crear_graficos_complementarios(df_procesado, curvas):
    """Crear gráficos complementarios de relaciones hidráulicas"""
...
- *Puede incluirse en el análisis*
...
if opcion == "🏠 Inicio":
    ...
    **🎯 Funcionalidades:**
    ...
    - **Opción para incluir GRUPO_ALTO_RH en el análisis**
    """)
        ...
        if archivo_subido is not None:
            try:
                ...
                if columnas_faltantes:
                    ...
                    # Procesar datos inicialmente (EXCLUYENDO GRUPO_ALTO_RH por defecto)
                    if st.button("🚀 Procesar Aforos", type="primary"):
                        with st.spinner("Procesando aforos y generando curvas..."):
                            try:
                                # Procesamiento inicial EXCLUYENDO GRUPO_ALTO_RH
                                curvas, clasificados = predecir_curvas_con_filtro_completo(modelo, df_subido, incluir_alto_rh=False)
                                
                                st.success(f"✅ {len(clasificados)} aforos procesados exitosamente (GRUPO_ALTO_RH excluido)")
                                ...
                                _, clasificados_completos = predecir_curvas_con_filtro_completo(modelo, df_subido, incluir_alto_rh=True)
                                    ...
                                    if st.button("🔄 Recalcular INCLUYENDO GRUPO_ALTO_RH"):
                                        with st.spinner("Recalculando curvas con GRUPO_ALTO_RH..."):
                                            try:
                                                ...
                                                curvas_con_alto_rh, clasificados_con_alto_rh = predecir_curvas_con_filtro_completo(modelo, df_subido, incluir_alto_rh=True)
...
# ... (las secciones de Ingreso Manual y Curvas se mantienen similares pero usando la nueva función)
