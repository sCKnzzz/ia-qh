...
# FUNCIONES PARA GRÁFICOS - CORREGIDAS PARA EXCLUIR GRUPO_ESTANDAR
def crear_grafico_principal(df, curvas, titulo):
    ...
    # Luego graficar las curvas (solo GRUPO_ALTO_RH y GRUPO_RECIENTE)
    for grupo, curva in curvas.items():
        ...
        # Agregar información del rango de niveles en la etiqueta
        rango_min = curva['rango_niveles'][0]
        rango_max = curva['rango_niveles'][1]
        label = f"{grupo} (R²={curva['r2']:.3f})\nNiveles: {rango_min:.1f}-{rango_max:.1f}m"
            ...
            try:
                ...
                if not columnas_faltantes:
                    ...
                    if st.session_state.procesamiento_realizado and st.session_state.curvas_sin_alto_rh is not None:
                        ...
                        # Mostrar ecuaciones con RANGOS DE NIVEL
                        st.subheader("📐 Ecuaciones y Rangos de Nivel (sin GRUPO_ALTO_RH)")
                        for grupo, curva in curvas_sin.items():
                            ...
                            with st.expander(f"{grupo} - R² = {curva['r2']:.3f} - Válido para niveles: {rango_min:.2f} a {rango_max:.2f} m"):
                                st.write(f"**Tipo de modelo:** {curva['nombre']}")
                                st.write(f"**Puntos utilizados:** {curva['n_puntos']}")
                                st.write(f"**Rango de niveles válido:** {rango_min:.2f} - {rango_max:.2f} m")
                        ...
                        if st.session_state.tiene_alto_rh:
                            ...
                            if st.button("🔄 RECALCULAR con GRUPO_ALTO_RH", key="btn_recalcular"):
                                with st.spinner("Recalculando con GRUPO_ALTO_RH..."):
                                    ...
                                    # Mostrar ecuaciones con RANGOS DE NIVEL
                                    st.subheader("📐 Ecuaciones y Rangos de Nivel (CON GRUPO_ALTO_RH)")
...
# ... (el resto del código para Ingreso Manual y Curvas se mantiene similar con las mismas exclusiones)
